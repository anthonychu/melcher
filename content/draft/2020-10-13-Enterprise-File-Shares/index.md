...
| Service Levels / Tiering | Standard 0.124354€/GiB (15.1k€ for 100TiB)
 Premium 0.248091€/GiB (30.1k€ for 100TiB)
 Ultra 0.331198€/GiB (40.2k€ for 100TiB) | Premium 0.162€/GiB+ 0.1375€/GiB Snapshots
 (19.3k€ for 90Tib data + 10TiB Snapshots)Dynamically change is not possible. |
| Service Level changable | Yes | No |
...
| Backup | Incremental Snapshots (4k block), Cross-Region Replication, 3rd party, native option planned | Incremental Snapshots (file), Azure Backup Integration |
| Snapshot Integration into &quot;Previous Version&quot; (Windows Client)SMB Client | Yes (Previous Versions + ~snapshot) | Yes (Previous Versions) |
| Snapshot Integration into NFS Client | Yes (~snapshot) | No |
| Integrated Snapshot Scheduling | Yes (Snapshot Policies) | No (scriptable via API) |
| Identity-based authentication and authorization | On-premises Active Directory Domain Services (AD DS) | Azure Active Directory (Azure AD)
...
|
 | **Azure NetApp Files + Global File Cache** | **Azure Files + Azure File Sync** |
| --- | --- | --- |
| Central File Locking | Yes | No |
| File Open Interleave | Yes | No |
| Optimized Transport | Yes | No |
| Hybrid storage support | Yes | No |
Rendered
...
Features
Feature	Azure NetApp Files	Azure Files Premium
| | | | | | | Native Azure Service, fully managed | Yes | Yes | | Protocol Compatibility | SMB 2.1/3.0/3.1.1, NFS 3/4.1 Multiprocotol: SMB+NFSv3 | FileREST, SMB 2.1/3.0, NFS 4.1 (Preview) | | Min Size | 4 TiB | 100 GiB | | Max Volume Size | 100 TiB | 100 TiB | | Max File Size | 16 TiB | 4 TiB | | Service Levels / Tiering | Standard 0.124354€/GiB (15.1k€ for 100TiB) Premium 0.248091€/GiB (30.1k€ for 100TiB) Ultra 0.331198€/GiB (40.2k€ for 100TiB) | Premium 0.162€/GiB+ 0.1375€/GiB Snapshots (19.3k€ for 90Tib data + 10TiB Snapshots)Dynamically change is not possible. | | Service Level changable | Yes | No | | Shape Capacity/Performance independently | Yes (Manual-QoS) | No | | On-Prem Access (Hybrid) | Yes (Express Route, VPN) | Yes (ExpressRoute, VPN, Internet) Private Link required (pricing) for VPN/ExpressRoute (Private Peering): €0.009 per GB In-/Outbound Data ProcessingOr ExpressRoute (Microsoft Peering). | | Regional Availability | 22+ regions | 32+ regions | | Regional Redundancy | LRS equivalent (99.99% SLA) | LRS (99.9% SLA) ZRS* (Asia Southeast, Australia East, Europe North, Europe West, US East, US East 2, US West 2) | | Geo Redundancy | Yes, Cross-Region Replication (Preview) | No | | Storage at-rest encryption | Yes (AES 256) | Yes (AES 256) | | Backup | Incremental Snapshots (4k block), Cross-Region Replication, 3rd party, native option planned | Incremental Snapshots (file), Azure Backup Integration | | Snapshot Integration into "Previous Version" (Windows Client)SMB Client | Yes (Previous Versions + ~snapshot) | Yes (Previous Versions) | | Snapshot Integration into NFS Client | Yes (~snapshot) | No | | Integrated Snapshot Scheduling | Yes (Snapshot Policies) | No (scriptable via API) | | Identity-based authentication and authorization | On-premises Active Directory Domain Services (AD DS) | Azure Active Directory (Azure AD) Azure Active Directory Domain Services (Azure AD DS) On-premises Active Directory Domain Services (AD DS)
...
IOPS (single volume/share)	Not limited explicitly, dependent on throughput & IO Size (benchmark ~460.000)Example:1 IOPS @ 64kb per provisioned GiB Premium8 IOPS @ 8k per provisioned GiB Premium	Baseline: 1 IOPS per provisioned GiB up to 100.000Burst: 3 IOPS per provisioned GiB up to 100.000
...
Hybrid Connectivity & Encryption
Feature	Azure NetApp Files	Azure Files Premium
| | | | | | | SMB signing | Yes | | | SMB in-flight encryption | No | Yes | | Active Directory Integration | Yes | Yes | | Azure Active Directory Independent | Yes | No (AD-Connect required) | | AD Kerberos Authentication | Yes (AES 256, AES128, DES) | Yes (AES 256) | | AD LDAP Signing | Yes | |
